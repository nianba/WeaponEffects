...
public static class SlashParticleEmitter
{
	...
	public static void EmitSwingTrailParticles(in WeaponSlashProfile profile, Vector2 position, Vector2 tangent, Vector2 outward, float strength = 0.16f)
	{
		...
		int count = CountSwingTrailParticles(in profile, strength);
		for (int i = 0; i < count; i++)
		{
			EmitSwingTrailParticle(in profile, position + Main.rand.NextVector2Circular(4f, 4f), tangent, outward);
		}
	}

	public static int CountSwingTrailParticles(in WeaponSlashProfile profile, float strength = 0.16f)
	{
		return Main.dedServ ? 0 : ScaledCount(profile.SwingParticles.Count, strength);
	}

	public static void EmitSwingTrailParticle(in WeaponSlashProfile profile, Vector2 position, Vector2 tangent, Vector2 outward)
	{
		if (Main.dedServ)
		{
			return;
		...
		Vector2 velocity = tangentDirection.RotatedBy(Main.rand.NextFloat(-particles.SpreadRadians * 0.28f, particles.SpreadRadians * 0.28f)) * Main.rand.NextFloat(1f, 2.6f) * particles.VelocityScale;
		velocity += outwardDirection * Main.rand.NextFloat(-0.12f, 0.32f) * particles.VelocityScale;
		Vector2 spawnPosition = position + Main.rand.NextVector2Circular(1.5f, 1.5f);
		if (particles.VisualStyle == SlashParticleVisualStyle.DrawnStar)
		{
